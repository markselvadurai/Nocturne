...
export type NightScore =
    { score: number; cloudDataAvailable: boolean }
...
export function computeScore(darknessHours: number, moonOverlapFraction: number, moonIlluminationFraction: number, clouds: CloudCoverResult): NightScore {
    const progress = Math.min(1, Math.max(0, (darknessHours - DARKNESS_MIN_HRS) / (DARKNESS_SAT_HRS - DARKNESS_MIN_HRS)));
