...
3. **The UTC day-seam bug.** Every date built in the evening carries *tomorrow's* UTC day; somewhere in the SunCalc chain the UTC day won, so every 7-night dot derived the next day's night. Every test passed — all test dates were authored at noon. Fix: anchor derived dates to local noon (`setHours(12,0,0,0)`), pinning day-identity. Lesson: test data that avoids the hostile region proves nothing about it. (Superseded by #11 — that fix was both incomplete and machine-dependent.)
...
11. **The day-seam again — and a fix that only worked in Toronto.** #3's anchor landed in `weekScores` and nowhere else: `tonightScores`, `selectSite()` and the `_selectedNight` seed still passed a raw `new Date()`, so after 20:00 EDT the markers and panel derived *tomorrow's* night under today's header. Worse, `setHours(12,0,0,0)` anchors to the **machine's** noon — correct only where the developer happens to sit, and wrong under `TZ=UTC`, which is what CI runs. The anchor moved into `getDarknessWindow`, resolved to noon in the **site's** zone. That fixed every caller at once but not the whole bug: `weekScores` still *generated* its seven dates from machine noon, so a reader more than twelve hours away — Tokyo, Sydney, Auckland — got a week starting a night early. Second pass: `siteToday(site, now)`, noon on the site's current calendar day, taking `now` as a parameter so it is testable without faking a clock (#1 again). Regression tests are authored with `Date.UTC` so they cannot pass on a lucky machine. Lesson: a bug that keeps returning is a modelling problem — “which night” was implicit in a `Date`, and implicit things get re-forgotten.
12. **Don't paraphrase a dependency's threshold — use it.** `getMoonOverlap` seeded its up/down state machine with `altitude >= 0.133`, but SunCalc counts the moon risen when its *upper limb* clears the horizon — centre altitude ≈ −0.35° once semidiameter and refraction are included. That 0.48° disagreement is a dead band: a window opening inside it inverts the parity of the entire walk, so a three-hour window two minutes after moonrise reported `overlapFraction 0.000` — a moonless night with the moon up throughout. The probe now computes SunCalc's own criterion (`altitude + semidiameter + 0.09 ≥ 0`, earth radius 6378.14, read from the package source rather than reconstructed). Roughly one bad night per site per year: invisible at seven sites, several wrong site-nights per refresh at three hundred.
13. **Tests are verified by breaking the code.** Every test written for #11 and #12 was mutation-checked — revert the fix, confirm the test fails, restore. It exposed two tests that asserted less than their names claimed: a DST case that passed whether the date arithmetic was calendar-aware or not, and a probe test blind to the semidiameter term until its window start moved to 30 seconds after moonrise. It also caught a broken harness reporting “zero failures” for a mutation already watched to fail — ANSI colour codes defeating a `grep` anchor. A test that has never failed is a claim, not evidence.
